...
(
    ...
    //SHIFT REG MODULES
    logic [DATA_WIDTH - 1:0] circ_buff_N_res;
    logic                    circ_buff_N_vld;

    circular_buffer_with_valid #(.width(DATA_WIDTH), .depth(N_ISQRT_STAGES)) circ_buff_N
    (
        .clk       ( clk             ),
        .rst       ( rst             ),
        .in_valid  ( arg_vld         ),
        .in_data   ( b               ),
        .out_valid ( circ_buff_N_vld ),
        .out_data  ( circ_buff_N_res )
    );

    logic [DATA_WIDTH - 1:0] circ_buff_2N_res;
    logic                    circ_buff_2N_vld;

    circular_buffer_with_valid #(.width(DATA_WIDTH), .depth(2 * N_ISQRT_STAGES + 1)) circ_buff_2N
    (
        .clk       ( clk              ),
        .rst       ( rst              ),
        .in_valid  ( arg_vld          ),
        .in_data   ( a                ),
        .out_valid ( circ_buff_2N_vld ),
        .out_data  ( circ_buff_2N_res )
    );
    ...
    always_ff @(posedge clk)
        if (isqrt1_vld)
            sum1_res <= isqrt1_out + circ_buff_N_res;
    ...
    always_ff @(posedge clk)
        if (isqrt2_vld)
            sum2_res <= isqrt2_out + circ_buff_2N_res;
